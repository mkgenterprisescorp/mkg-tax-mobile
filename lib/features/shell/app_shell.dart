...
class AppShell extends StatelessWidget {
  ...
  static const _tabs = <({String path, String label, IconData icon})>[
    ...
    (path: '/forms', label: 'Forms', icon: Icons.dashboard_customize_outlined),
    (path: '/banking', label: 'Banking', icon: Icons.account_balance_outlined),
    (path: '/blogs', label: 'Blogs', icon: Icons.article_outlined),
  ];
  ...
  int _indexFor(String location) {
    ...
    // Nested feature routes still highlight Forms as the work hub.
    return 2;
  }
  ...
  String _titleFor(String location) {
    ...
    if (location.startsWith('/tessa')) return 'TESSA AI';
    ...
    if (location.startsWith('/financial')) return 'FINANCIAL';
    if (location.startsWith('/account')) return 'ACCOUNT OVERVIEW';
    if (location.startsWith('/banking')) return 'BANKING';
    if (location.startsWith('/blogs')) return 'BLOGS';
    return 'FORMS LIST';
  }
            ...
            DrawerHeader(
              ...
              child: Column(
                ...
                children: const [
                  ...
                  SizedBox(height: 12),
                  Text('Demo Client', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 18)),
                  Text('demo@mkgenterprisescorp.com', style: TextStyle(color: Colors.white70)),
                ],
              ),
            ),
            item(Icons.dashboard_outlined, 'Forms / Dashboard', '/forms'),
            item(Icons.folder_outlined, 'Documents', '/documents'),
            item(Icons.assignment_outlined, 'Tax Organizer', '/organizer'),
            item(Icons.receipt_long_outlined, 'Billing', '/billing'),
